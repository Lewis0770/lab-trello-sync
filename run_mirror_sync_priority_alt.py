#!/usr/bin/env python3
"""
Trello Card Bi-Directional Mirroring Script - Priority III & II Alternative
...
1. Cards in "Priority III" or "Priority II" lists, OR
2. Cards with "In-Progress" checklist ≥ 50% complete
3. Cards must NOT have "Completed" label (case-insensitive)

Runs every other Monday at 8 AM Eastern via GitHub Actions.
"""
...
# Constants
PRIORITY_LIST_NAMES = ["Priority III", "Priority II"]
PROGRESS_CHECKLIST_NAME = "In-Progress"
COMPLETED_LABEL_NAME = "Completed"
COMPLETION_THRESHOLD = 0.50  # Changed to 50%

# Mirror metadata markers
MIRROR_MARKER = "🤖 MIRROR_METADATA_ALT:"  # Different marker to distinguish from main script
MIRROR_COMMENT_MARKER = "[Bot-Alt] Mirrored from"
...
def should_mirror_card(api: TrelloAPI, card: Dict, board_labels: Dict[str, str]) -> bool:
    """Determine if a card should be mirrored based on Priority III/II OR 50% completion criteria"""
    # Skip if card has "Completed" label
    if has_completed_label(card, board_labels):
        print(f"🚫 Skipping '{card['name']}' - has Completed label")
        return False
    
    # Check if card is in Priority III or Priority II list
    list_name = get_list_name(api, card["idList"])
    is_priority_list = any(list_name.lower() == priority.lower() for priority in PRIORITY_LIST_NAMES)
    
    if is_priority_list:
        print(f"✅ '{card['name']}' - in {list_name} list")
        return True
    
    # Check if card has In-Progress checklist ≥ 50% complete
    ...
    if progress > 0:
        print(f"⏳ Skipping '{card['name']}' - In-Progress checklist only {progress:.1%} complete (needs ≥50%)")
    else:
...
def clear_alt_mirror_cards(api: TrelloAPI, list_id: str, list_name: str) -> None:
    """Clear only cards that were created by this alternative script"""
    response = api.get(f"lists/{list_id}/cards", {
        "attachments": "true",
        "checklists": "all",
        "members": "true",
    ...
    for card in cards:
        # Check if this card has our alternative mirror metadata
        if MIRROR_MARKER in card.get("desc", ""):
    ...
    print(f"🧹 Cleared {deleted_count} alt-mirror cards from {list_name} list")

def sync_changes_from_master(api: TrelloAPI) -> int:
    """Sync changes from master board back to source boards (alt mirror cards only)"""
    ...
    for list_id, board_name in [(MASTER_PROPOSALS_LIST_ID, "Proposals"), (MASTER_PAPERS_LIST_ID, "Papers")]:
        ...
        for card in cards:
            ...
            if metadata:
                ...
                # Only show warning for cards that don't have any mirror metadata (neither original nor alt)
                if "🤖 MIRROR_METADATA:" not in card.get("desc", ""):
                    print(f"⚠️  No mirror metadata found for card '{card['name']}'")
    
    print(f"🔄 Phase 1 Complete: Synced {synced_count} alt-mirror cards back to source boards")
    return synced_count
...
def main():
    ...
    print("🚀 Starting Trello Card Bi-Directional Mirror Script - Priority III & II Alternative")
    print(f"⏰ Started at: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}")
    print(f"🎯 Targeting Priority III & II lists OR ≥50% In-Progress checklist completion")
    
    # Initialize API
    api = TrelloAPI(TRELLO_API_KEY, TRELLO_TOKEN)
    
    # Phase 1: Sync changes from master back to source boards (alt mirror cards only)
    synced_count = sync_changes_from_master(api)
    
    # Phase 2: Clear alt-mirror cards from master board lists
    print("\n🧹 Phase 2: Clearing alt-mirror cards from Master Board lists...")
    clear_alt_mirror_cards(api, MASTER_PROPOSALS_LIST_ID, "Master Proposals")
    clear_alt_mirror_cards(api, MASTER_PAPERS_LIST_ID, "Master Papers")
    
    # Phase 3: Mirror qualifying cards from source boards
    print("\n📤 Phase 3: Mirroring qualifying Priority III & II cards to Master board...")
    total_mirrored = 0
    ...
    print(f"\n🎉 Alt-Mirror Script completed successfully!")
    print(f"🔄 Alt-mirror cards synced back to source: {synced_count}")
